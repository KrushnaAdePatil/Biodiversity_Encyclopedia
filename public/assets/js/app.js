/* ==========================================================================
   BioSphere Encyclopedia — vanilla JavaScript single-page application
...
var I18N = {
  en: { search: "Search species, scientific names, habitats…", day: "Species of the Day", facts: "Did You Know?", trending: "Trending species", explore: "Explore", results: "results", clear: "Clear all", read: "Read the full profile →", noResults: "No species matched those filters." },
  mr: { search: "प्रजाती, शास्त्रीय नाव, अधिवास शोधा…", day: "आजची प्रजाती", facts: "तुम्हाला माहीत आहे का?", trending: "लोकप्रिय प्रजाती", explore: "शोधा", results: "निकाल", clear: "सर्व साफ करा", read: "संपूर्ण माहिती वाचा →", noResults: "कोणतीही प्रजाती सापडली नाही." },
  hi: { search: "प्रजाति, वैज्ञानिक नाम, आवास खोजें…", day: "आज की प्रजाति", facts: "क्या आप जानते हैं?", trending: "लोकप्रिय प्रजातियाँ", explore: "खोजें", results: "परिणाम", clear: "सब हटाएँ", read: "पूरी जानकारी पढ़ें →", noResults: "कोई प्रजाति नहीं मिली।" }
};
...
function speciesCard(s, view) {
  ...
  var imgWrap = '<div class="sp-img-wrap" data-wiki="' + esc(s.wiki_title || "") + '" data-slug="' + esc(s.slug) + '" data-emoji="' + esc(s.emoji || "🐾") + '" data-alt="' + esc(s.common_name) + '"></div>';
  if (view === "list") {
...
function speciesGrid(items, view) {
  if (!items || !items.length) {
    return '<div class="panel empty-state"><div class="es-emoji">🔍</div><p>' + esc(t("noResults")) + '</p><a class="btn btn-primary" href="#/explore">' + esc(t("clear")) + "</a></div>";
  }
...
var MOSAIC = [
  { slug: "bengal-tiger", wiki: "Bengal_tiger", emoji: "🐅", name: "Bengal Tiger", tall: true },
  { slug: "indian-peafowl", wiki: "Indian_peafowl", emoji: "🦚", name: "Indian Peafowl" },
  { slug: "blue-morpho", wiki: "Morpho_peleides", emoji: "🦋", name: "Blue Morpho" },
  { slug: "lotus", wiki: "Nelumbo_nucifera", emoji: "🪷", name: "Sacred Lotus" }
];

function renderHome() {
  setTitle("BioSphere Encyclopedia — Every living thing on Earth");
  app.innerHTML =
    ...
    '<span class="hero-tag">🌍 Open biodiversity atlas</span>' +
    "<h1>Every living thing on Earth, in one place</h1>" +
    '<p class="hero-sub">From the smallest soil microbe to the blue whale — deep profiles with live photo galleries, Marathi and Hindi names, IUCN status, diet, lifespan, distribution and astonishing facts. Built with plain HTML, CSS and JavaScript, powered by a Python + PostgreSQL backend.</p>' +
    '<div class="hero-cta"><a class="btn btn-primary" href="#/explore">Start exploring →</a>' +
    '<a class="btn btn-outline-w" href="#/quiz">Take the ID quiz</a>' +
    '<a class="btn btn-outline-w" href="#/map">Open the atlas</a></div>' +
    '<div class="hero-stats" id="hero-stats">' + ["Profiles", "Kingdoms", "Countries", "Threatened"].map(function (x) { return '<div class="hero-stat"><b>–</b><span>' + x + "</span></div>"; }).join("") +
    '</div></div>' +
    ...
    '<div class="section-head reveal"><div><p class="eyebrow">Browse by kingdom</p><h2 class="section-h">Fifteen great branches of life</h2></div><a class="section-link" href="#/explore">Advanced explorer →</a></div>' +
    ...
    "</div></section>" +
    '<section class="container" style="padding-top:38px">' +
    '<div class="section-head reveal"><div><p class="eyebrow">Most visited</p><h2 class="section-h">' + esc(t("trending")) + '</h2></div><a class="section-link" href="#/explore?sort=popular">See all →</a></div>' +
    ...
    [["#/compare", "⚖️", "Comparison tool", "Place up to three species side by side — size, weight, lifespan and status."],
     ["#/map", "🗺️", "Interactive atlas", "Click a continent or ocean to see which species live there."],
     ["#/quiz", "🧠", "Identification quiz", "Can you name the species from one photo and one clue?"],
     ["#/checklist", "✅", "My life list", "Track everything you have personally spotted in the wild."]].map(function (tw) {
      return '<a class="panel tool-card card-hover" href="' + tw[0] + '"><span class="t-emoji">' + tw[1] + "</span><h3>" + tw[2] + "</h3><p>" + tw[3] + '</p><span class="t-open">Open →</span></a>';
    }).join("") +
    "</div></section>" +
    '<section class="container" style="padding-top:38px"><div class="conservation-strip" id="cons-strip">' +
    '<div class="big-globe">🌍</div><p class="eyebrow" style="color:#bbf7d0">Why this matters</p>' +
  ...
  apiCached("/stats").then(function (st) {
    ...
    if (hs) {
      ...
      $$("b", hs).forEach(function (b, i) { b.textContent = vals[i] != null ? vals[i] : "–"; });
    }
    var ct = $("#cons-title");
  ...
  apiCached("/categories").then(function (cats) {
    ...
    box.innerHTML = cats.map(function (c) {
      return '<a class="cat-card card-hover" style="background:linear-gradient(140deg,' + c.accent_from + "," + c.accent_to + ')" href="#/category/' + c.slug + '">' +
        ...
        '<span class="count">' + (c.species_count || 0) + " profiles · " + esc(c.known_species) + "</span></a>";
    }).join("");
    lazyBackgrounds(box);
  ...
  api("/day").then(function (d) {
    ...
    s.innerHTML = '<div class="panel spot-card">' +
      ...
      (sp.name_mr || sp.name_hi ? '<p class="local-names">' + (sp.name_mr ? "मराठी: " + esc(sp.name_mr) : "") + (sp.name_mr && sp.name_hi ? " · " : "") + (sp.name_hi ? "हिंदी: " + esc(sp.name_hi) : "") + "</p>" : "") +
      '<p class="muted">' + esc(sp.summary) + "</p>" +
      '<div class="spot-stats">' +
      [["Size", (sp.size_summary || "—").split(";")[0]], ["Lifespan", sp.lifespan_wild ? sp.lifespan_wild + " yrs" : "—"], ["Habitat", (sp.habitats || [])[0] || "—"], ["Range", (sp.continents || [])[0] || "—"]].map(function (kv) {
  ...
  api("/facts").then(function (d) {
    ...
    el.innerHTML = d.items.map(function (f) {
      return '<a class="panel fact-card card-hover" href="#/species/' + f.slug + '"><span class="f-emoji">' + f.emoji + "</span><span><p>" + esc(f.fact) + "</p><span class=\"f-name\">" + esc(f.common_name) + " →</span></span></a>";
    }).join("");
  }).catch(function () {});
...
var SORTS = [["popular", "Most popular"], ["az", "Name A–Z"], ["za", "Name Z–A"], ["largest", "Largest"], ["smallest", "Smallest"], ["longest-lived", "Longest lived"], ["rarest", "Rarest first"]];

function renderExplore(q) {
  setTitle("Explore species · BioSphere");
  apiCached("/facets").then(function (facets) {
    ...
    app.innerHTML = '<div class="container page"><div class="panel empty-state"><div class="es-emoji">🔌</div><p>Backend unavailable. The Python API must be running.</p></div></div>';
  });
}
...
function loadExplore(q, facets) {
  ...
  api("/species?" + params.join("&")).then(function (data) {
    ...
    app.innerHTML = '<div class="container page"><div class="panel empty-state"><div class="es-emoji">⚠️</div><p>Could not load species.</p></div></div>';
  });
}

function drawExplore(q, facets, data, view, kingdoms) {
  setTitle((q.q ? 'Results for "' + q.q + '" · ' : "") + "Explore · BioSphere");
  ...
  app.innerHTML = '<div class="container page">' +
    '<p class="crumb"><a href="#/">Home</a> / <span>Explore</span>' + (q.q ? ' / “' + esc(q.q) + '”' : "") + "</p>" +
    "<h1 class=\"section-h\" style=\"font-size:clamp(26px,4vw,38px)\">" + (q.q ? "Results for “" + esc(q.q) + "”" : "Explore the living world") + "</h1>" +
    ...
    '<span class="view-toggle"><button data-view="grid" class="' + (view === "grid" ? "on" : "") + '">▦</button><button data-view="list" class="' + (view === "list" ? "on" : "") + '">☰</button></span>' +
    "</span></div>" +
    ...
    multi("Conservation status", "conservation", facets.conservation, function (v) { var l = (LEGEND[v] || {}).label || v; return v + " · " + l; }) +
    multi("Diet", "diet", facets.diets) +
    multi("Habitat", "habitat", facets.habitats) +
    multi("Continent / ocean", "continent", facets.continents) +
    '<div class="f-group"><p class="f-title">A–Z index</p><div class="letters">' + "ABCDEFGHIJKLMNOPQRSTUVWXYZ".split("").map(function (l) {
    ...
    '<button data-page="' + Math.min(pages, page + 1) + '"' + (page >= pages ? " disabled" : "") + ">Next ›</button></nav></div>";
...
function drawSpecies(s, related, sightings) {
  setTitle(s.common_name + " (" + s.scientific_name + ") · BioSphere");
  ...
  var heroImg = '<figure class="sp-hero-img"><div class="wrap sp-img-wrap" data-wiki="' + esc(s.wiki_title || "") + '" data-slug="' + esc(s.slug) + '" data-emoji="' + esc(s.emoji || "🐾") + '" data-alt="' + esc(s.common_name) + '"></div>' +
  ...
  app.innerHTML =
    ...
    '<div class="container hero-grid"><div>' +
      ...
      (s.name_mr || s.name_hi ? '<p class="local-names">' + (s.name_mr ? "<strong>मराठी:</strong> " + esc(s.name_mr) + " &nbsp; " : "") + (s.name_hi ? "<strong>हिंदी:</strong> " + esc(s.name_hi) : "") + "</p>" : "") +
      '<p style="max-width:660px">' + esc(s.summary) + "</p>" +
      '<div class="action-row">' +
        '<button class="btn btn-ghost btn-sm" id="speak-btn">🔊 Listen</button>' +
        '<button class="btn btn-ghost btn-sm" id="share-btn">🔗 Share</button>' +
        '<button class="btn btn-ghost btn-sm" id="lifelist-btn">+ Life list</button>' +
        '<a class="btn btn-ghost btn-sm" href="#/compare?a=' + esc(s.slug) + '">⚖️ Compare</a>' +
      "</div>" +
      '<div class="quick-stats">' +
      [["Size", (s.size_summary || "—").split(";")[0]], ["Weight", s.weight_kg ? s.weight_kg.toLocaleString() + " kg" : "—"], ["Lifespan", s.lifespan_wild ? s.lifespan_wild + " yrs wild" : "—"], ["Population", s.population || "—"]].map(function (kv) {
    ...
    '<div class="container detail-layout"><div>' +
      section("overview", "📖 Overview",
        ...
        sizeBar(s)) +
      section("gallery", "📸 Photo gallery", '<div id="gallery-box"><div class="skeleton" style="aspect-ratio:16/10;border-radius:18px"></div></div>') +
      section("taxonomy", "🧬 Scientific classification",
      ...
      section("physical", "📏 Physical description",
        '<p style="margin-top:0"><strong>Measurements:</strong> ' + esc(s.size_summary || "—") + "</p>" + kvGrid(s.physical) +
        (s.colors && s.colors.length ? '<p style="margin:14px 0 6px"><strong>Colours:</strong> ' + pills(s.colors, "") + "</p>" : "")) +
      section("diet", "🍽️ Diet &amp; feeding",
      ...
      section("lifespan", "⏳ Lifespan &amp; growth",
        '<div class="grid grid-2">' + [["In the wild", s.lifespan_wild], ["In human care", s.lifespan_captive]].map(function (kv) {
          return '<div class="panel-muted" style="padding:16px"><p class="f-title">' + kv[0] + '</p><p style="font-size:30px;font-weight:800;margin:0">' + (kv[1] || "—") + ' <small style="font-size:13px;font-weight:400" class="muted">years</small></p></div>';
        }).join("") + "</div>" +
      ...
      section("habitat", "🌍 Habitat &amp; distribution",
        ...
        '<div class="panel-muted" style="padding:16px;font-size:13.5px"><p style="margin:0"><strong>Countries:</strong> ' + esc((s.countries || []).join(", ") || "—") + "</p>" +
      ...
      (s.reproduction && s.reproduction.length ? section("reproduction", "🔄 Reproduction", kvGrid(s.reproduction)) : '<span id="reproduction"></span>') +
      (s.behavior && s.behavior.length ? section("behaviour", "🧠 Behaviour &amp; characteristics", kvGrid(s.behavior) +
      ...
      (subsp.length ? section("subspecies", "🧬 Subspecies &amp; varieties",
      ...
      section("threats", "⚠️ Threats &amp; conservation",
        ...
        ((s.threats || []).length ? '<div class="kv-grid" style="display:grid;gap:8px">' + s.threats.map(function (x) {
          return '<div class="panel-muted" style="padding:11px 14px;font-size:13.5px">⚠️ ' + esc(x) + "</div>";
        }).join("") + "</div>" : "") +
      ...
      section("facts", "🤔 " + facts.length + " amazing facts",
        '<div class="fact-grid">' + facts.map(function (f) { return '<div class="fact-li">' + esc(f) + "</div>"; }).join("") + "</div>") +
      section("related", "🔗 Related species", '<div id="rel-grid">' + speciesGrid(related) + "</div>") +
      section("community", "🗣️ Community sightings &amp; reviews", '<div id="sight-zone"></div>') +
      "</div>" +
      '<aside><div class="panel sidebar-card"><p class="sc-title">Quick facts</p><ul class="sidebar-facts">' +
      [["Kingdom", tax.kingdom], ["Family", tax.family], ["Diet", s.diet_type], ["Activity", s.activity], ["Speed", s.speed || "—"], ["Habitat", (s.habitats || [])[0] || "—"], ["Status", status.label]].map(function (kv) {
        return '<li><span class="k">' + kv[0] + '</span><span class="v">' + esc(kv[1] || "—") + "</span></li>";
      }).join("") + "</ul></div>" +
      '<div class="panel sidebar-card"><p class="sc-title">External references</p><ul class="ext-links">' +
      '<li><a href="https://en.wikipedia.org/wiki/' + encodeURIComponent(s.wiki_title || "") + '" target="_blank" rel="noreferrer noopener">Wikipedia →</a></li>' +
      '<li><a href="https://www.gbif.org/species/search?q=' + encodeURIComponent(s.scientific_name) + '" target="_blank" rel="noreferrer noopener">GBIF occurrence records →</a></li>' +
      '<li><a href="https://www.iucnredlist.org/search?query=' + encodeURIComponent(s.scientific_name) + '" target="_blank" rel="noreferrer noopener">IUCN Red List →</a></li>' +
      '<li><a href="https://www.inaturalist.org/search?q=' + encodeURIComponent(s.scientific_name) + '" target="_blank" rel="noreferrer noopener">iNaturalist observations →</a></li></ul></div></aside>' +
      "</div>";
    ...
    .then(function (d) {
      ...
      if (!d || !d.images || !d.images.length) {
        box.innerHTML = '<div class="grid grid-3">' + [0, 1, 2].map(function (i) {
          return '<div style="position:relative;aspect-ratio:4/3;border-radius:14px;overflow:hidden">' + plate(s.slug + i, s.emoji || "🐾") + "</div>";
        }).join("") + '<p class="muted" style="grid-column:1/-1;font-size:12px">Live photo feed unavailable — illustrated specimen plates shown instead.</p></div>';
        return;
      }
      var index = 0;
      var shots = d.images;
      var render = function () {
        ...
        box.innerHTML = '<div class="gallery-view">' +
          ...
          '<span class="gallery-counter">📸 ' + (index + 1) + " / " + shots.length + "</span>" +
          '<button class="gal-btn gal-prev" aria-label="Previous photo">‹</button>' +
          '<button class="gal-btn gal-next" aria-label="Next photo">›</button>' +
          '<div class="gallery-cap"><span>' + esc(shot.caption || s.common_name) + "</span><small>© " + esc(shot.credit) + " · Wikimedia Commons</small></div></div>" +
...
function openLightbox(shot) {
  ...
  lb.innerHTML = '<img alt="" src="' + (shot.originalUrl || shot.url) + '" referrerpolicy="no-referrer" /><button class="lightbox-close">Close ✕</button>';
...
function drawSightings(s, items) {
  ...
  zone.innerHTML = '<div class="community-grid">' +
    '<form class="panel" style="padding:18px" id="sight-form">' +
      ...
      '<textarea name="note" rows="3" placeholder="Behaviour, time of day, habitat…" maxlength="1000"></textarea>' +
      ...
      '<span class="rating-row" id="rating-row">' + [1, 2, 3, 4, 5].map(function (n) { return '<button type="button" data-n="' + n + '" class="' + (n <= 5 ? "on" : "") + '">⭐</button>'; }).join("") + "</span></div>" +
    ...
    "<div><div style=\"display:flex;justify-content:space-between;align-items:baseline;margin-bottom:8px\">" +
      ...
      (avg ? '<span class="muted" style="font-size:13px">⭐ ' + avg + " · " + items.length + " reports</span>" : "") + "</div>" +
      '<div id="sight-list">' +
      (items.length ? items.slice(0, 10).map(function (it) {
        return '<div class="panel sight-item"><div class="sight-head"><b>' + esc(it.observer) + "</b>" +
          (it.location ? '<span class="loc">📍 ' + esc(it.location) + "</span>" : "") +
          '<span class="stars">' + "⭐".repeat(it.rating || 5) + "</span></div>" +
          (it.note ? '<p class="sight-note">' + esc(it.note) + "</p>" : "") +
          '<p class="muted" style="font-size:10.5px;margin:4px 0 0">' + esc(String(it.created_at || "").slice(0, 10)) + (it.seen ? " · seen in the wild" : " · not yet seen") + "</p></div>";
      }).join("") : '<div class="panel-muted empty-state"><div class="es-emoji">👀</div><p>No sightings logged yet. Be the first!</p></div>') +
      "</div></div></div>";
  var rating = 5;
  ...
  $("#sight-form").onsubmit = function (e) {
    ...
    fetch("/py/sightings", {
      ...
      toast("Sighting submitted — thank you!");
...
function updateLifeListBtn(s) {
  ...
  btn.textContent = on ? "✓ On my life list" : "+ Life list";
  btn.onclick = function () {
    var list = lifeList();
  ...
  Promise.all([apiCached("/categories"), api("/subcategories?category=" + encodeURIComponent(slug))]).then(function (res) {
    ...
    api("/species?" + params).then(function (data) {
      ...
      app.innerHTML =
        ...
        '<p style="opacity:.9;margin:0">' + esc(cat.name_mr) + " · " + esc(cat.name_hi) + "</p>" +
        ...
        '<p class="muted" style="font-size:13.5px">Science has described roughly <strong>' + esc(cat.known_species) + "</strong> species in this group. We publish the most significant and most searched species first, then expand outward — every profile is written and checked against published sources.</p>" +
...
/* ------------------------------------------------------------------ atlas */
var REGIONS = [
  ["North America", "🦅", "#2563eb", "1 / 3", "1 / 3"], ["Europe", "🦊", "#7c3aed", "4 / 6", "1 / 2"],
  ["Asia", "🐅", "#ea580c", "6 / 9", "1 / 3"], ["Africa", "🦁", "#ca8a04", "4 / 6", "2 / 4"],
  ["South America", "🦜", "#16a34a", "2 / 4", "3 / 5"], ["Australia & Oceania", "🦘", "#0d9488", "7 / 9", "3 / 5"],
  ["Antarctica", "🐧", "#0ea5e9", "3 / 7", "5 / 6"], ["Oceans", "🐋", "#0369a1", "1 / 2", "4 / 6"]
];
function renderAtlas(q) {
  var region = q.region || "Asia";
  setTitle("Interactive biodiversity atlas · BioSphere");
  api("/atlas?region=" + encodeURIComponent(region)).then(function (d) {
    ...
    app.innerHTML = '<div class="container page">' +
      ...
      '<div class="section-head"><h2 class="section-h" style="font-size:22px">' + esc(region) + '</h2><a class="section-link" href="#/explore?continent=' + encodeURIComponent(region) + '">Open in explorer →</a></div>' +
      speciesGrid(d.items) + "</div>";
    lazyImages(app); reveal(app);
  }).catch(function () {
    app.innerHTML = '<div class="container page"><div class="panel empty-state"><div class="es-emoji">🧭</div><p>Atlas unavailable right now.</p></div></div>';
  });
}

/* ------------------------------------------------------------------ compare */
function renderCompare(q) {
  setTitle("Species comparison · BioSphere");
  api("/species?perPage=200&sort=az").then(function (all) {
    ...
    app.innerHTML = '<div class="container page"><div class="panel empty-state"><div class="es-emoji">⚖️</div><p>Could not load the comparison data.</p></div></div>';
  });
}

function drawCompare(options, selected, q) {
  var optionHtml = function (sel) {
    return '<option value="">Choose a species…</option>' + options.map(function (o) {
  ...
  var textRows = [
    ...
    ["Speed", function (s) { return esc(s.speed || "—"); }],
    ["Status", function (s) { return statusBadge(s.conservation, true); }],
    ["Population", function (s) { return esc(s.population || "—"); }],
    ...
    (selected.length ?
      ...
      nums.map(function (row) {
        ...
        return "<tr><th>" + row[0] + "</th>" + selected.map(function (s) {
          ...
          return "<td><b>" + (v ? v.toLocaleString() + " " + row[2] : "—") + '</b><div class="cmp-bar-track"><div class="cmp-bar" style="width:' + (v / max) * 100 + '%"></div></div></td>';
        }).join("") + "</tr>";
      }).join("") +
      textRows.map(function (row) {
        return "<tr><th>" + row[0] + "</th>" + selected.map(function (s) { return "<td>" + row[1](s) + "</td>"; }).join("") + "</tr>";
      }).join("") + "</tbody></table></div>"
      : '<div class="panel empty-state"><div class="es-emoji">⚖️</div><p>Pick a species to begin comparing.</p></div>') +
    "</div>";
...
/* ------------------------------------------------------------------ quiz */
function renderQuiz() {
  setTitle("Identify the species quiz · BioSphere");
  ...
  api("/quiz?count=8").then(function (d) {
    ...
    app.innerHTML = '<div class="container page"><div class="panel empty-state quiz-shell"><div class="es-emoji">🧠</div><p>Could not load the quiz.</p><button class="btn btn-primary" onclick="location.reload()">Retry</button></div></div>';
  });
}
...
function drawQuiz(st) {
  ...
  if (st.i >= qs.length) {
    ...
    var verdict = pct === 100 ? "Perfect — you are a walking field guide!" : pct >= 75 ? "Excellent naturalist instincts." : pct >= 50 ? "Solid effort — keep exploring." : "Time for a wander through the encyclopedia!";
    app.innerHTML = '<div class="container page"><div class="quiz-shell panel" style="padding:40px;text-align:center">' +
      '<div style="font-size:56px">' + (pct >= 75 ? "🏆" : pct >= 50 ? "🌿" : "🔍") + "</div>" +
  ...
  app.innerHTML = '<div class="container page"><div class="quiz-shell panel">' +
    ...
    '<div class="quiz-body">' +
      (st.hint || st.picked
        ? '<p class="quiz-hint">💡 ' + esc(q.hint) + (st.picked ? '<span class="sci">' + esc(q.scientificName) + "</span>" : "") + "</p>"
      ...
      (st.picked ? '<div style="display:flex;gap:10px;align-items:center;margin-top:16px;flex-wrap:wrap"><b>' + (st.picked === q.answer ? "✅ Correct!" : "❌ Not quite.") + '</b><a class="section-link" href="#/species/' + q.answer + '">Read the profile</a><button class="btn btn-primary btn-sm" id="quiz-next" style="margin-left:auto">' + (st.i + 1 === qs.length ? "See results" : "Next →") + "</button></div>" : "") +
    "</div></div></div>";

  var imgWrap = $("#quiz-img");
  if (!st.picked) imgWrap.dataset.mystery = "1";
  attachImage(imgWrap, q.wikiTitle, q.slug, st.picked ? q.emoji : "❓", "Mystery species");
...
function renderChecklist() {
  setTitle("My life list · BioSphere");
  api("/species?perPage=200&sort=az").then(function (d) {
    ...
    var draw = function (query) {
      ...
      app.innerHTML = '<div class="container page">' +
        ...
        '<input class="sort-select" id="life-query" style="width:100%;margin-bottom:14px;border-radius:12px" placeholder="Search the encyclopedia to add a species…" value="' + esc(query || "") + '" />' +
    ...
    app.innerHTML = '<div class="container page"><div class="panel empty-state"><div class="es-emoji">✅</div><p>Could not load the encyclopedia list.</p></div></div>';
  });
}

/* ------------------------------------------------------------------ contribute */
function renderContribute() {
  setTitle("Contribute a species · BioSphere");
  apiCached("/categories").then(function (cats) {
    ...
    $("#contrib-form").onsubmit = function (e) {
      ...
      fetch("/py/contribute", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) })
        ...
        .then(function () {
          app.innerHTML = '<div class="container page"><div class="panel empty-state" style="max-width:640px;margin:0 auto"><div class="es-emoji">🌱</div><h2 class="section-h">Submission received</h2><p class="muted">Thank you — your contribution is stored in the editorial queue. Submissions with sources are published fastest.</p><a class="btn btn-primary" href="#/contribute">Submit another</a></div></div>';
        })
...
/* ------------------------------------------------------------------ static pages */
var GLOSSARY = [
  ["Classification", [["Binomial nomenclature", "The two-part Latin naming system — genus then species, e.g. Panthera tigris."], ["Taxon", "Any named group of organisms at any rank, from kingdom to subspecies."], ["Endemic", "Found naturally in one area and nowhere else, like the Southern Birdwing in the Western Ghats."], ["Subspecies", "A geographically distinct population within a species."], ["Cultivar", "A plant variety produced by selective breeding."]]],
  ["Ecology", [["Keystone species", "A species whose removal would collapse its ecosystem."], ["Apex predator", "A predator with no natural enemies as an adult."], ["Trophic cascade", "A chain reaction through a food web from a top predator."], ["Mutualism", "A relationship where both partners benefit."], ["Biodiversity hotspot", "A region with exceptional endemism under severe threat — India has four."]]],
  ...
  ["Anatomy", [["Carapace", "The hard upper shell of a turtle or crustacean."], ["Baleen", "Keratin filter plates some whales use to strain krill."], ["Chromatophore", "A pigment cell enabling colour change."], ["Prehensile", "Capable of grasping — as in a seahorse's tail."], ["Epiphyte", "A plant growing on another for support, like most orchids."]]],
...
function renderGlossary() {
  setTitle("Glossary of scientific terms · BioSphere");
  apiCached("/facets").then(function (facets) {
...
var FAQS = [
  ["Is BioSphere Encyclopedia free?", "Yes — no paywall, no accounts, no advertising trackers. Teachers and students can use every feature freely."],
  ...
  ["What stack powers the site?", "A hand-written HTML, CSS and JavaScript frontend; a Python (FastAPI) backend; and a PostgreSQL database — with Next.js acting as the static host and reverse proxy."],
  ...
  ["Can I cite this site in a school project?", "Yes — cite the species page URL and access date. For academic work, also follow the GBIF and IUCN links on each profile."],
  ...
  ["Will you cover all two million described species?", "Not all at once — we publish the most searched and ecologically significant species first, then work outwards. Depth beats breadth."]
];

function renderFaq() {
  setTitle("FAQ · BioSphere");
...
function renderAbout() {
  ...
  app.innerHTML = '<div class="container page prose" style="max-width:820px">' +
    ...
    '<p class="muted">Around two million species have been formally described by science, and researchers estimate between 8 and 20 million more are waiting. BioSphere Encyclopedia makes that diversity understandable — one carefully written, beautifully illustrated profile at a time.</p>' +
    '<h2>The stack behind this page</h2>' +
    '<div class="grid grid-2">' +
    [["🖥️", "Frontend", "Hand-written HTML, CSS and JavaScript — a single-page app with a hash router, lazy image loading, dark mode and full keyboard accessibility."],
     ["🐍", "Backend", "Python with FastAPI serving 18 REST endpoints — species search, filters, galleries, quizzes, sightings and contributions."],
     ["🐘", "Database", "PostgreSQL stores 179 species profiles, 15 kingdoms, 90 sub-groups, sightings and community contributions."],
     ["🧩", "Reverse proxy", "Next.js serves this static frontend at the root and transparently proxies /py/* to the Python service."]].map(function (c) {
    ...
    "<h2>Principles</h2><ul><li><strong>Accuracy before volume</strong> — every profile is written against published sources (IUCN, GBIF, peer review).</li><li><strong>Local names matter</strong> — Marathi and Hindi names sit beside the Latin binomial.</li><li><strong>Living media</strong> — photos stream from Wikimedia Commons; illustrated plates cover the gaps.</li><li><strong>Accessible to everyone</strong> — keyboard support, text scaling, high contrast, text-to-speech.</li><li><strong>Free forever</strong> — no paywall, no tracking walls.</li></ul>" +
...
function renderPrivacy() {
  setTitle("Privacy & terms · BioSphere");
...
function renderNotFound(what) {
  setTitle("Not found · BioSphere");
  app.innerHTML = '<div class="container page"><div class="panel empty-state" style="max-width:620px;margin:30px auto">' +
    '<div class="es-emoji">🔭</div><h1 class="section-h" style="font-size:30px">Species not found</h1>' +
    (what ? '<p class="muted">Nothing matches <b>' + esc(what) + '</b>.</p>' : "") +
    '<p class="muted">This page may have gone extinct — around 80% of the world\'s species are still undescribed.</p>' +
...
function initHeader() {
  ...
  var drawMega = function (cats) {
    megaMenu.innerHTML = '<div class="mega-grid">' + cats.map(function (c) {
      return '<a class="mega-cat" href="#/category/' + c.slug + '"><span class="cat-emoji" style="background:linear-gradient(135deg,' + c.accent_from + "," + c.accent_to + ')">' + c.emoji + "</span><span><span class=\"cat-name\">" + esc(c.name) + '</span><span class="cat-meta">' + esc(c.known_species) + " · " + (c.species_count || 0) + " profiles</span></span></a>";
    }).join("") + "</div>";
  ...
  menuBtn.onclick = function () { mobileMenu.hidden = !mobileMenu.hidden; menuBtn.textContent = mobileMenu.hidden ? "☰" : "✕"; };
  apiCached("/categories").then(function (cats) {
    $("#mobile-cats").innerHTML = cats.map(function (c) { return '<a href="#/category/' + c.slug + '">' + c.emoji + " " + esc(c.name) + "</a>"; }).join("");
  }).catch(function () {});
  mobileMenu.addEventListener("click", function (e) {
    if (e.target.tagName === "A") { mobileMenu.hidden = true; menuBtn.textContent = "☰"; }
  });
  ...
  $("#footer-copy").textContent = "© " + new Date().getFullYear() + " BioSphere Encyclopedia · Frontend: HTML + CSS + JS · Backend: Python (FastAPI) · Database: PostgreSQL · Photos: Wikimedia Commons";
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if ("serviceWorker" in navigator && location.protocol === "https:") {
    // Optional PWA hook — registration only if a worker exists.
  }
});
})();
